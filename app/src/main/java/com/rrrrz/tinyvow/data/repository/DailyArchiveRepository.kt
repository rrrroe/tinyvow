...
class DailyArchiveRepository(
    ...
    private suspend fun buildArchiveGroupConfigs(
        ...
        return groupIds.mapNotNull { groupId ->
            ...
            ArchiveGroupConfig(
                id = groupId,
                name = snapshot?.groupName ?: current?.name ?: return@mapNotNull null,
                type = snapshot?.groupType ?: current?.type ?: return@mapNotNull null,
                limitPeriod = snapshot?.limitPeriod ?: current?.limitPeriod ?: return@mapNotNull null,
                limitMinutes = snapshot?.limitMinutes ?: current?.limitMinutes ?: return@mapNotNull null,
                bonusMinutes = bonusMinutes,
                pointsPerMinute = snapshot?.pointsPerMinute ?: current?.pointsPerMinute ?: 0.0,
                encourageMetric = if (snapshot != null) EncourageMetric.APP_USAGE else current?.encourageMetric ?: EncourageMetric.APP_USAGE,
