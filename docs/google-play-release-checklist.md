...
- 隐私政策 URL：`https://tinyvow.rorolo.com/privacy.html`
- 账号删除 URL：`https://tinyvow.rorolo.com/account-delete.html`
...
- 安全实践：第一版不启用 Tiny Vow 自有服务端传输；用户可以主动导出本地数据摘要或可恢复备份，也可以在应用内清除本地数据。可恢复备份不包含微信读书 API Key 明文；导出文件只在用户通过系统分享面板主动发送时离开设备。
...
- 无障碍服务：仅用于检测前台应用切换，并在已设置限额的应用超额时显示 Tiny Vow 的阻断页面。服务配置为 `canRetrieveWindowContent=false`，不读取屏幕文字，不代替用户点击，不修改系统设置。需要在 Play Console 完成 Accessibility API 声明。
